...
{
	system_param.softwave_version_marker=0x01;
	system_param.hardwave_version_marker=0x01;
	system_param.decice_id=0x04;
}
uint32_t device_param_read(uint8_t* buf,uint8_t fuc_param, uint32_t len)
{
 u32 data_len;
  ...
  buf[i++] = system_param.hardwave_version_marker;//硬件版本
	buf[i++] = system_param.softwave_version_marker;//软件版本
	buf[i++] = system_param.decice_id;//设备类型
...
//	buf[i++] = system_param.change_RTC_SetAlarm_ok;//(iap_param.ChipUniqueID[1]>>8)&0xFF;	
	...
	buf[i++] = system_param.hardwave_version_marker;//iap_param.ChipUniqueID[4]&0xFF;
	buf[i++] = system_param.softwave_version_marker;//(iap_param.ChipUniqueID[4]>>8)&0xFF;
	...
	buf[i++] = system_param.decice_id;//iap_param.version;
	buf[i++] = Get_Sum8(buf + 1, data_len - 3);
	buf[i++] = COMM_PROTOCOL_TAIL;
	return i;
}
...
void app_param_default(void)
{
  memset(&system_param, 0, sizeof(system_param));
	...
	system_param.softwave_version_marker=0x03;
	system_param.hardwave_version_marker=0x02;
	system_param.decice_id=0x04;
...
void It_Judge(void)
{
	...
	if(RTC_WakeUp_It == 1)//12S INTO
	{
		Watchdog_Refresh(); //每12s喂狗
		RTC_WakeUp_It=0;
		run_time += RTC_WakeUp_Period;//运行时间统计
